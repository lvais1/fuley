...
import { Flame, Zap, Trophy, TrendingUp, Target, Calendar, Brain } from 'lucide-react';
import Navigation from '@/components/Navigation';
import { useAppStore } from '@/lib/store';
import { generateBodyInsights } from '@/lib/body-insights';
...
const INSIGHT_TYPE_STYLES = {
  pattern: { color: 'var(--color-green)',  bg: 'rgba(0,255,148,0.08)',  border: 'rgba(0,255,148,0.2)' },
  warning: { color: 'var(--color-orange)', bg: 'rgba(255,107,53,0.07)', border: 'rgba(255,107,53,0.2)' },
  tip:     { color: '#8B92A5',             bg: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.08)' },
};

export default function HistoryPage() {
  const { foodEntries, stats, lastReadiness, user } = useAppStore();
  const [activeTab, setActiveTab] = useState<'overview' | 'insights' | 'log'>('overview');
  ...
  const bodyInsights = generateBodyInsights(foodEntries, lastReadiness, stats, user);
    ...
    <div className="min-h-screen pb-28" style={{ background: 'var(--color-bg)' }}>
      <div className="px-5 pt-14">
        ...
        <div className="flex gap-1 p-1 rounded-2xl mb-6 bg-[rgba(255,255,255,0.04)]">
          {(['overview', 'insights', 'log'] as const).map(tab => (
            <button
              key={tab}
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="w-16 h-1 rounded-full bg-[rgba(255,255,255,0.06)] mt-1 overflow-hidden">
                                <div className="h-full rounded-full" style={{ width: `${progress}%`, background: 'var(--color-text-3)' }} />
                              </div>
                            </div>
                          </div>
                        );
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: '0 0 30px rgba(0,255,148,0.06)',
                }}
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-sm font-black">Your Body Insights</p>
                    <p className="text-[11px]" style={{ color: 'var(--color-text-3)' }}>
                      AI-powered patterns from your fuel history
                    </p>
                  </div>
                </div>
                <p className="text-xs leading-relaxed" style={{ color: 'var(--color-text-2)' }}>
                  The more you log, the smarter these insights become. They&apos;re generated from your actual nutrition patterns — not generic advice.
                </p>
              </motion.div>

              {/* Insights list */}
              <div className="space-y-3">
                {bodyInsights.map((insight, i) => {
                  const style = INSIGHT_TYPE_STYLES[insight.type];
                  return (
                    <motion.div
                      key={insight.id}
              ...
              {/* Insight count context */}
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.5 }}
                className="text-center text-xs mt-6 px-6"
                style={{ color: 'var(--color-text-3)' }}
              >
                Based on {foodEntries.length} logged meals. Log more consistently to unlock deeper patterns.
              </motion.p>
            </motion.div>
          )}
